...
- Cadastro de rifas por usuarios logados com aprovacao administrativa antes de publicar.
...
Para criar ou redefinir uma conta admin automaticamente na Vercel, configure:
...
ADMIN_SETUP_TOKEN=um-token-secreto
```

Depois envie um `POST` para:

```text
https://seu-dominio.vercel.app/api/admin/ensure-default-admin
```

com o header:

```text
x-admin-setup-token: um-token-secreto
```

Essa rota confirma o email, cria/atualiza o usuario no Supabase e marca `id_admin = true`.
