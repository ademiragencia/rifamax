...
async function findCompraByReference(supabase, externalReference) {
...
async function updateSupabasePurchase(payment) {
  ...
  const compraAtual = await findCompraByReference(supabase, externalReference)

  if (!compraAtual) {
    return { skipped: false, error: 'Compra nao encontrada para esta referencia.' }
  }
