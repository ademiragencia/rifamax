...
export default function App() {
  ...
  const [paymentMethodLoading, setPaymentMethodLoading] = useState(null)
  ...
  async function confirmarCompra(paymentMethod = 'mercado_pago') {
    if (!user) {
      setAuthMode('login')
      goTo('auth')
    ...
    const metodoPagamento = paymentMethod === 'pix' ? 'pix' : 'mercado_pago'

    setPaymentLoading(true)
    setPaymentMethodLoading(metodoPagamento)
    ...
    try {
      ...
      if (isRemoteMode) {
        ...
        const compraResponse = await fetch('/api/compras/create', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        const compraResult = await compraResponse.json().catch(() => ({}))

        if (!compraResponse.ok) {
          throw new Error(compraResult.error || 'Erro ao reservar numeros.')
        }

        const compra = compraResult.compra
        if (!compra?.id) {
          ...
          <aside className="summary-panel">
            ...
            <div className="payment-actions">
              <button
                ...
                type="button"
                onClick={() => confirmarCompra('pix')}
                disabled={selectedNumbers.length === 0 || paymentLoading}
              >
                <QrCode size={17} aria-hidden="true" />
                {paymentMethodLoading === 'pix' ? 'Gerando Pix...' : 'Pagar com Pix'}
              </button>
              <button
                className="outline-button full"
                type="button"
                onClick={() => confirmarCompra('mercado_pago')}
                disabled={selectedNumbers.length === 0 || paymentLoading}
              >
                <CircleDollarSign size={17} aria-hidden="true" />
                {paymentMethodLoading === 'mercado_pago' ? 'Abrindo checkout...' : 'Cartao, saldo ou boleto'}
              </button>
            </div>
            <p className="payment-note">Pix e outras formas abrem no ambiente seguro do Mercado Pago.</p>
          </aside>
        </section>
      </main>
    )
